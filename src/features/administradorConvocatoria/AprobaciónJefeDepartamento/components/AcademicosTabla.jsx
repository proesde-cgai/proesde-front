...
export default function AcademicosTabla({ academicos, setAcademicos, codigoAutoridad }) {
    ...
    // Resetear estado cuando cambia el código de autoridad (nuevo jefe buscado)
    useEffect(() => {
        // Si cambia el codigoAutoridad, resetear el estado de agregar académico
        setIsAdding(false);
        setNewCodigo("");
    ...
    const handleAcceptNew = async () => {
        if (!newStatus.found || !newData || !codigoAutoridad) return;

        // Guardar en la API
        setNewStatus({ ...newStatus, loading: true });
        const result = await guardarAcademico(codigoAutoridad, newCodigo);
        ...
        if (result.success) {
            ...
            let errorMessage = "Error al guardar el académico. Por favor intente nuevamente.";
            if (result.message) {
                // Si el mensaje contiene "Debe desactivar", cortar antes de esa parte
                const debeDesactivarIndex = result.message.indexOf("Debe desactivar");
                if (debeDesactivarIndex !== -1) {
                    errorMessage = result.message.substring(0, debeDesactivarIndex).trim();
                } else {
    ...
    const handleDesactivar = (academico) => {
        console.log("academico desactivado:", academico);
    ...
    const confirmDesactivar = async () => {
        ...
        console.log("idJefeDepartamento a eliminar:", deleteConfirm.codigoAutoridad);
        const result = await desactivarAcademico(deleteConfirm.codigoAutoridad, deleteConfirm.codigoAcademico);

        if (result.success) {
            // Actualizar la lista local removiendo el académico desactivado
            setAcademicos(academicos.filter((a) => a.id !== deleteConfirm.id));
            setDeleteConfirm(null);
        } else {
    ...
    const academicosActivos = academicos.filter(a => a.estado !== false);
                    ...
                    <table className={styles.table}>
                        ...
                        <tbody>
                            {academicosActivos.map((academico, index) => (
                                <tr
                                    key={academico.id || index}
                                    className={index % 2 === 0 ? styles.tableRowEven : styles.tableRowOdd}
                                >
                                    <td className={styles.tableCell}>
                                        {academico.codigoAcademico}
                                    </td>
                                    <td className={styles.tableCell}>
                                        {academico.nombreCompletoAcademico}
                                    </td>
                                    <td className={styles.tableCell}>
                                ...
                                <table className={styles.modalInfoTable}>
                                    ...
                                    <tbody>
                                        <tr className={styles.modalInfoBodyRow}>
                                            <td className={styles.modalInfoValueCell}>{deleteConfirm.codigoAcademico}</td>
                                            <td className={styles.modalInfoValueCell}>{deleteConfirm.nombreCompletoAcademico}</td>
