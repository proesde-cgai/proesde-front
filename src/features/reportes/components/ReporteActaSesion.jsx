...
const ReporteActaSesion = () => {
  ...
  const handleSubmitActaAcuerdos = async (data) => {
    ...
    console.log("data.horaFecha ", data.horaFecha);
    ...
    try {
      ...
      generateActaSesionPdf(response.data);
  ...
  const generateActaSesionPdf = (response) => {
    const blob = new Blob([response], { type: "application/pdf" });
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `ACTA_ACUERDOS.pdf`);
          ...
          <form action="" onSubmit={handleSubmit(handleSubmitActaAcuerdos)}>
            <div>
              <p className={styles.tituloParrafo}>
                ...
                />
                ACTA DE ACUERDOS DE LA {comsion.replace(/\bnull\b/g, "").replace(/\(.*?\)/g, "").trim()}
              </p>
